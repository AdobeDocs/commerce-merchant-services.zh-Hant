...
_含標籤的Experience Platform聯結器資料流程_
...
- 安裝 [Adobe Commerce Events SDK](https://github.com/adobe/commerce-events/tree/main/packages/commerce-events-sdk). 有關PHP儲存區域，請參閱 [安裝](install.md) 主題。 如需PWA Studio店面的相關資訊，請參閱 [PWA Studio指南](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/aep/).
...
店面事件包含的設定檔資訊是根據 `personalEmail` （適用於帳戶事件）和 `identityMap` （適用於所有其他店面事件）欄位。 Experience Platform聯結器會根據這兩個欄位加入及產生設定檔。 但是，每個欄位在建立設定檔時需要遵循的不同步驟：
...
下列步驟顯示如何設定 `pageView` 事件與 `identityMap` 在Experience Platform聯結器中：
...
與使用ECID來身分識別及連結設定檔資訊的店面事件不同，後台事件資料以SaaS為基礎，因此沒有ECID可用。 對於後台活動，您需要使用電子郵件以專門識別購物者。 在本節中，您將瞭解如何使用電子郵件將後台事件資料連結至ECID。

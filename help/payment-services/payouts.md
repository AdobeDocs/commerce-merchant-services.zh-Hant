...
>[!NOTE]
>
>付款報表只會顯示已擷取的訂單(付款作業設為 [`Authorize and Capture`](https://experienceleague.adobe.com/docs/commerce-merchant-services/payment-services/get-started/production.html#set-payment-services-as-payment-method)) — 或 [標示為 `Invoiced`](https://docs.magento.com/user-guide/sales/invoice-create.html).

## 支付資料視覺效果檢視

支付資料視覺化檢視可在支付服務首頁取得。 這是詳細表格每日彙總金額的視覺化表示法 [支付報表檢視](#payouts-report-view).
...
1. 在 _管理員_ 側欄，前往 **[!UICONTROL Sales]** > **[!UICONTROL Payment Services]**. 「付款」資料視覺效果檢視會顯示在「付款」區段中。
...
* 如果您在「即時」模式中有任何存放區（一或多個），資料來源選項會預設為 _[!UICONTROL Live]_.
* 報表匯出一律遵循資料來源選擇。
...
| [!UICONTROL Pay method] | 信用卡型別 — *[!UICONTROL BANK]*， *[!UICONTROL PAYPAL]*， *[!UICONTROL CREDIT_CARD]* — 和相關聯的卡片提供者(例如 *Visa* 或 *萬事通*) |
| [!UICONTROL TRANS AMT] | 交易金額 |
| [!UICONTROL CUR] | 交易金額的貨幣單位 |
| [!UICONTROL PENDING] | 尚未支付的金額 |
| [!UICONTROL CUR] | 暫緩金額的貨幣單位 |
| [!UICONTROL SELLER AMT] | 轉給客戶或從客戶轉出的資金金額 <br> <br>資金移出賣家帳戶會顯示破折號(-)首碼。 |
...
| [!UICONTROL CHBK FEE] | 與交易相關的借項衝回費用 <br> <br>破折號(-)首碼表示已迴轉借項衝回費用。 |

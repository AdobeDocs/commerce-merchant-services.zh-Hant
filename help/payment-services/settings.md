...
1. 更改 [稍後付費消息](payments-options.md#pay-later-button)，切換 **[!UICONTROL Show PayPal Pay Later message]** 的雙曲餘切值。
1. 要啟用調試模式，請切換 **[!UICONTROL Debug Mode]** 選擇器。
...
您還可以配置 _[!UICONTROL Button style]_付款按鈕的選項：

1. 更改 **[!UICONTROL Layout]**&#x200B;選中 `Vertical` 或 `Horizontal`。

   >[!NOTE]
   >
   > 如果按鈕樣式配置為 `Horizontal` 您的商店配置為顯示多個付款按鈕，您只能看到產品頁、結帳頁和迷你購物車上顯示的兩個按鈕，以及購物車中顯示的一個按鈕。

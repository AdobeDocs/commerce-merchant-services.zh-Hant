...
[!DNL Data Connection]擴充功能會將您的Adobe Commerce Web執行個體連線至Adobe Experience Platform和Edge Network。 對於行動應用程式開發人員而言，您可以搭配Commerce使用Adobe Experience Platform Mobile SDK來擷取Commerce資料，並將其傳送至Experience Platform。 [深入瞭解](./mobile-sdk-epc.md)。
...
Adobe Experience Platform提供了一套技術，可在與Commerce商店中的資料結合後，透過Edge Network將該資料發佈到其他AdobeDX產品，以解鎖有關購物者購買行為的洞察資訊。 透過這些深入分析，您可以跨所有管道建立更加個人化的購物體驗。

下圖顯示當安裝及設定[!DNL Data Connection]擴充功能時，您的Commerce資料如何從您的存放區傳輸至其他AdobeDX產品。
...
使用[!DNL Data Connection]擴充功能將您的Commerce資料傳送至Experience Platform，是Commerce資料共用功能的一部分。 另一邊是選用的擴充功能，稱為[Audience Activation](https://experienceleague.adobe.com/docs/commerce-admin/customers/audience-activation.html)。 此擴充功能可讓您在Real-Time CDP中建立受眾，並將這些受眾部署至您的Commerce商店，以告知購物車價格規則、相關產品規則及動態區塊。
...
在您設定Commerce之間的連線以Experience Platform和Experience Platform到Commerce後，資料會繼續流動。 除非升級時需要，否則您不需要重新連線。
...
1. 確定您有Experience Platform](https://experienceleague.adobe.com/docs/experience-platform/collection/permissions.html)中資料收集的[許可權。
1. 檢閱您可以收集及傳送的[資料型別](data-ingestion.md)。
1. 使用Commerce特定的欄位群組建立或更新您的[時間序列事件結構描述](update-xdm.md)或[設定檔記錄資料結構描述](profile-data.md)。
1. [根據您建立或更新的結構描述建立資料集](https://experienceleague.adobe.com/docs/platform-learn/implement-mobile-sdk/experience-cloud/platform.html#create-a-dataset)。 此資料集包含傳送至Experience PlatformEdge的Commerce資料。
...
本指南的其餘部分將更詳細地引導您完成所有這些步驟，以便您快速入門，並開始使用Commerce商店中AdobeDX產品的強大功能。

...
<tbody><tr>
<td><strong>已啟用</strong></td>
<td>此電子郵件會傳送給客戶，通知他們已在商戶商店取消訂單。 將設為 <code>No</code> 以停用電子郵件通知。 如果電子郵件範本已停用，此功能不會阻止訂單被取消。</td>
<td>存放區檢視</td>
<td>否</td>
</tr>
<tr>
...
<tbody><tr>
<td><strong>已啟用</strong></td>
<td>此電子郵件會傳送給客戶，通知他們商戶商店已取消部分訂單。 將設為 <code>No</code> 以停用電子郵件通知。 如果電子郵件範本已停用，並不會阻止部分取消訂單。</td>
<td>存放區檢視</td>
<td>否</td>
</tr>
<tr>
...
<td>以彙總報表的形式傳送給指定的商戶人員的電子郵件，此彙總報表會包含所有在商戶商店中無法撿料的未結訂單，直到其存貨可用為止。 </br></br> 商戶可以使用此報表來啟動及管理儲存至儲存庫的存貨移轉或補貨。 </br></br>此通知僅適用於 [!DNL Ship-to-Store] 功能已啟用。
...
>[!NOTE]
>
>如果您允許延期交貨，則必須提供管理員電子郵件地址，以接收有關這些訂單的通知。 將位址新增至下列組態設定： **[!UICONTROL Send Order Delayed Email Copy To]** 在 [訂單延遲](#order-delayed) 範本，和 [!UICONTROL Ship To Store Email Recipients] 在 [收貨商店](#ship-to-store) 範本。
